...
echo -e "感谢使用 “\033[32m 宝塔Anankke/SSPanel-Uim 快速部署工具 \033[0m”"
echo "----------------------------------------------------------------------------"
echo -e "请注意这些要求:“\033[31m 宝塔版本=6.0+ \033[0m”，添加网址PHP版本必须选择为“\033[31m PHP7.4 \033[0m”,添加完成后地址不要改动！"
echo "----------------------------------------------------------------------------"
stty erase '^H' && read -p "请输入宝塔面板添加的网站域名,请不要修改添加之后的默认地址（例如:www.baidu.com，不带http/https）：" website
stty erase '^H' && read -p "请输入宝塔面板添加的网站名称：" webname
...
sleep 1
##下载解压拷贝源码
echo -e "${Info} 正在解压处理程序源码"
git clone -b dev https://github.com/Anankke/SSPanel-Uim.git temp && mv temp/.git . && rm -rf temp && git reset --hard
...
sed -i "s/https:\/\/sspanel.host\//http:\/\/$website/g" /www/wwwroot/$website/config/.config.php
sed -i "s/\['db_host'\]      = ''/\['db_host'\]      = 'localhost'/g" /www/wwwroot/$website/config/.config.php
sed -i "s/SSPanel-UIM/$webname/g" /www/wwwroot/$website/config/.config.php
sed -i "s/NimaQu/$sspanelmukey/g" /www/wwwroot/$website/config/.config.php
sed -i "s/\['db_database'\]  = 'sspanel'/\['db_database'\]  = '$mysqldatabase'/g" /www/wwwroot/$website/config/.config.php
sed -i "s/\['db_username'\]  = 'root'/\['db_username'\]  = '$mysqlusername'/g" /www/wwwroot/$website/config/.config.php
sed -i "s/\['db_password'\]  = 'sspanel'/\['db_password'\]  = '$mysqlpassword'/g" /www/wwwroot/$website/config/.config.php
...
echo "0 0 * * * php -n /www/wwwroot/$website/xcat Job DailyJob" >> /var/spool/cron/root
echo "*/1 * * * * php /www/wwwroot/$website/xcat Job CheckJob" >> /var/spool/cron/root
